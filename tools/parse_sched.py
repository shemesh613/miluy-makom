...
PERSONAL_BLOCKS = {
    ...
    # אישר בעצמו 17.8: הגן נמשך מעבר ל-13:15, ולכן גם שעה 7 אינה זמינה.
    # שעה 6 (גנים) כבר מסומנת בקובץ עצמו.
    'הרב שלומי': {'hours': ['7'],
                  'reason': 'עדיין בגן ב-13:15'},
    # חמישי 4–5: יוצא לקניות. בקובץ זה נראה חלון פנוי, והוא לא בבניין.
    'הרב שלומי חמישי': {'teacher': 'הרב שלומי', 'hours': ['4', '5'], 'days': [4],
                        'reason': 'חמישי 4–5 — יוצא לקניות'},
}

blocked = 0
for key, rule in PERSONAL_BLOCKS.items():
    name = rule.get('teacher', key)
    t = parsed.get(name)
    if not t:
